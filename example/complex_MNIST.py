...
def run(cluster_used, job_name, task_index, venv, protocol_used):
    FLAGS = tf.app.flags.FLAGS
    server = tf.train.Server(cluster_used,
    ...
    learning_rate = 0.001
    training_epochs = 10
    logs_path = "./conv_mnist/" + str(len(cluster_used[workers]))
    ...
    if job_name == "ps":
        ...
        with tf.device(tf.train.replica_device_setter(
                worker_device="/job:worker/task:%d" % task_index, cluster=cluster_used)):
